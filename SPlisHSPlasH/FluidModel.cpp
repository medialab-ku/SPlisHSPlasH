...
{		
	...
	addField({ "acceleration_boundary", FieldType::Vector3, [&](const unsigned int i) -> Real* { return &getAccelerationBoundary(i)[0]; }, true });
	addField({ "acceleration_total", FieldType::Vector3, [&](const unsigned int i) -> Real* { return &getAccelerationTotal(i)[0]; }, true });
}
...
FluidModel::~FluidModel(void)
{
	...
	removeFieldByName("acceleration_boundary");
	removeFieldByName("acceleration_total");
...
void FluidModel::reset()
{
	...
	for (unsigned int i = 0; i < nPoints; i++)
	{
		...
		getAccelerationBoundary(i).setZero();
		getAccelerationTotal(i).setZero();
		m_density[i] = 0.0;
		m_particleId[i] = i;
...
void FluidModel::resizeFluidParticles(const unsigned int newSize)
{
	...
	m_a_boundary.resize(newSize);
	m_a_total.resize(newSize);
...
void FluidModel::performNeighborhoodSearchSort()
{
	...
	d.sort_field(&m_a_boundary[0]);
	d.sort_field(&m_a_total[0]);
